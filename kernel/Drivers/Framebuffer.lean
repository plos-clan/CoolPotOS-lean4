...
def responseFramebufferCount (response : RawAddr) : UInt64 :=
  load64 (response.value + 8)

def responseFramebuffers (response : RawAddr) : RawAddr :=
  { value := load64 (response.value + 16) }

def firstFramebuffer (response : RawAddr) : RawAddr :=
  { value := load64 (responseFramebuffers response).value }

def address (fb : RawAddr) : RawAddr := { value := load64 fb.value }

def width (fb : RawAddr) : UInt64 := load64 (fb.value + 8)

def height (fb : RawAddr) : UInt64 := load64 (fb.value + 16)

def pitch (fb : RawAddr) : UInt64 := load64 (fb.value + 24)
