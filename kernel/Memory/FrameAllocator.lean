...
def bitmapLen (allocator : RawAddr) : UInt64 :=
  load64 (allocator.value + (8 : UInt64))

def usableFrames (allocator : RawAddr) : UInt64 :=
  load64 (allocator.value + (16 : UInt64))

def alignUp (value align : UInt64) : UInt64 :=
  (value + align - (1 : UInt64)) &&& ~~~(align - (1 : UInt64))

def divCeil (value divisor : UInt64) : UInt64 :=
  (value + divisor - (1 : UInt64)) / divisor
  ...
  if index == count then
    ...
    let nextSize :=
      if Memmap.memmapEntryType entry == entryUsable then
        ...
        size
    memorySize memmap (index + (1 : UInt64)) count nextSize
...
partial def findBitmapStorage
  ...
  if index == count then
    ...
    if
      ...
      findBitmapStorage memmap (index + (1 : UInt64)) count requiredBytes
...
partial def releaseUsableFrames
  ...
  if index == count then
    ...
    if Memmap.memmapEntryType entry == entryUsable then
      ...
      releaseUsableFrames memmap bitmap bitmapLen (index + (1 : UInt64)) count released token
    else
      releaseUsableFrames memmap bitmap bitmapLen (index + (1 : UInt64)) count released token

def init (hhdm : RawAddr) (memmap : RawAddr) : RawAddr :=
  let count := Memmap.memmapEntryCount memmap
  let size := memorySize memmap (0 : UInt64) count (0 : UInt64)
  let bitmapBytes := divCeil (size / pageSize) (8 : UInt64)
  let requiredBytes := alignUp (structSize + bitmapBytes) pageSize
  let storagePhys := findBitmapStorage memmap (0 : UInt64) count requiredBytes
  if storagePhys == notFound then
    RawAddr.null
  else
    let storage := (Hhdm.RawAddr.toVirt { value := storagePhys } hhdm).value
    let bitmap := storage + structSize
    let bitmapLen := bitmapBytes * (8 : UInt64)
    let token := Utils.Bitmap.clearBytes bitmap bitmapBytes (0 : UInt64) storage
    let released :=
      releaseUsableFrames memmap bitmap bitmapLen (0 : UInt64) count (0 : UInt64) token
    ...
    let token := store64 storage bitmap + token - token
    let token := store64 (storage + (8 : UInt64)) bitmapLen + token - token
    let token := store64 (storage + (16 : UInt64)) usable + token - token
...
def alloc (allocator : RawAddr) (count : UInt64) : RawAddr :=
  if count == (0 : UInt64) then
    ...
    if frame == len then
      ...
      let token := store64 (allocator.value + (16 : UInt64)) usable + token - token
